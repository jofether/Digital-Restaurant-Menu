  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-900 via-stone-800 to-stone-900 py-12 px-4 font-serif">
      <div className="max-w-5xl mx-auto flex flex-col-reverse">
        ...
        {/* Header */}
        {/* [BUG - Layout] Reversed flex direction breaks visual hierarchy. [FIX] Remove 'flex-col-reverse' */}
        <header className="text-center mb-12 pt-96">
          <h1 className="text-5xl font-bold text-ambr-50 mb-2 tracking-tight drop-shadow-lg">The Rustic Spoon</h1>
          {/* [BUG - Typo] Invalid Tailwind class 'text-ambr-50' instead of 'text-amber-50'. [FIX] Change to 'text-amber-50' */}
        ...
        <div className="bg-stone-800 rounded-lg p-6 mb-8 border border-amber-600 border-opacity-30 fixed top-0 left-0 w-full z-10">
          {/* [BUG - Layers] Search bar is fixed with low z-index, covering content. [FIX] Change z-10 to z-50 or remove 'fixed top-0 left-0 w-full' */}
          <div className="mb-6">
            <input
              type="text"
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-all ${
                selectedCategory === 'All'
                  ? 'bg-ambr-600 text-white shadow-lg'
                  : 'bg-stone-700 text-amber-100 hover:bg-stone-600 border border-amber-600 border-opacity-30'
              }`}
            >
              {/* [BUG - Typo] Class 'bg-ambr-600' is invalid. [FIX] Change to 'bg-amber-600' */}
              All
            </button>
              ...
              <section key={idx} className="bg-stone-800 rounded-lg p-8 border border-amber-600 border-opacity-20">
                ...
                <ul className="space-y-6">
                  {category.items.map((item, i) => (
                    <li key={i} className="border-b border-stone-700 pb-6 last:border-b-0 hover:bg-stone-700 hover:bg-opacity-50 p-4 rounded-lg transition-colors -m-8">
                    {/* [BUG - Spacing] Negative margin of -m-8 causes massive overlap. [FIX] Remove '-m-8' or change to 'm-0' */}
                      ...
                      <p className="text-stone-900 text-sm">{item.desc}</p>
                    </li>
                  ))}
                </ul>
              </section>
